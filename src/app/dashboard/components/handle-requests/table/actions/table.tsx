...
const Form = ({
  ...
  return (
    <form
      onSubmit={(ev) => {
        ev.preventDefault();
				const inputInvalid = document.getElementById('has-food-invalid-page') as HTMLInputElement;
				if (currentRequest.foods.some((item) => {
					return (
						!item.cooperativeId ||
						(item.cityHallFoodId === "none" &&
							!item.price &&
							!item.weight &&
							!item.name?.length) || !item.quantity
					);
				})) {
					inputInvalid.setCustomValidity("Algumas páginas possuem alimentos sem algumas informações!");
