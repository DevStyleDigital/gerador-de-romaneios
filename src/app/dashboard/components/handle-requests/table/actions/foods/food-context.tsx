...
const FoodContext = React.createContext(
	{} as {
		...
		setFood: React.Dispatch<
			React.SetStateAction<Partial<FoodRequest> | undefined>
		>;
...
export const FoodProvider = ({
	...
	const foodSelected = foods.find(
		(item) => defaultFood.cityHallFoodId === item.cityHallFoodId,
	);

	const [food, setFood] = React.useState<Partial<Food> | undefined>(
		foodSelected,
	);
	const [issue, setIssue] = React.useState<string | null>(defaultFood.issue);
