...
const Signature = ({ texts }: { texts: JSX.Element[] }) => (
  <View style={styles.signatureContainer}>
		<View style={{ marginBottom: "12px", display: 'flex', flexDirection: 'row' }}>
			...
			<Text style={{ width: '100%', textTransform: 'uppercase', fontSize: '14px', fontWeight: 'bold', backgroundColor: '#f1f1f1', padding: '0 4px' }}>
				{ texts.map((item, i) => i === texts.length - 1 ? item : <>{item} - </>) }
			</Text>
		</View>
...
export const HtmlRequestTemplate = ({ request }: { request: RequestTypeDetailed }) => {
    ...
    <Page size="A4" style={styles.page}>
      <View style={[styles.borderBottom, { position: "relative" }]}>
        ...
        <Text
          style={[
            ...
            { fontSize: 12, top: '-20px', margin: "8px auto 12px", position: "absolute" },
          ]}
        >
      ...
      <View style={styles.table}>
        <View style={[styles.tableRow, { backgroundColor: "#e1e1e1" }]}>
          ...
          <Text style={[styles.col2, styles.entityInfoTitle]}>
            Ocorrências
          </Text>
          <Text style={[styles.col3, styles.entityInfoTitle]}>
            Unidade (Tipo)
          </Text>
          <Text style={[styles.col4, styles.entityInfoTitle]}>
            Quantidade
          </Text>
        </View>
        {request.foods.map((food, i) => {
          return (
            ...
            >
              <Text style={[styles.col1, { fontSize: '14px' }]}>{food.name}</Text>
              ...
              <Text style={[styles.col4, { fontSize: '14px', textAlign: 'center', fontWeight: 'bold' }]}>{food.quantity?.toFixed(1)}</Text>
            </View>
          );
        })}
      </View>
      ...
      <Signature texts={getTextBetweenTags(request.school.comments, request.cooperative.id)}/>
      <Text style={{ position: 'absolute', bottom: '4px', right: 24 }} render={({ pageNumber, totalPages }) => (
...
export const PDFRequests = ({ requests }: { requests: RequestTypeDetailed[] }) => {
  return <Document>
    {requests.map((request, i) => <HtmlRequestTemplate key={i.toString()} request={request} />)}
  </Document>
}